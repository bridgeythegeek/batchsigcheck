...
class BatchSigCheck:
    ...
    def run(self):
        ...
        if len(os.listdir(self.lnk_dir)) > 0:
            ...
            logger.info('Writing SQLite3 database...')
            sqlite_file = os.path.join(self.out_dir, '{}_BatchSigCheck.sqlite'.format(self.now))
            logger.info('SQlite3 database file: %s' % sqlite_file)
            try:
                conn = sqlite3.connect(sqlite_file)
                c = conn.cursor()

                logger.info('Creating and inserting data for SigCheck...')
                # SigCheck - table
                headers = [h.replace(' ', '_') for h in result[0]]
                sql = 'CREATE TABLE sigcheck ({});'.format(','.join(['{} TEXT'.format(h) for h in headers]))
                c.execute(sql)
                # SigCheck - data
                result = result[1:]
                for row in result:
                    fixed_row = row
                    fixed_row[2] = datetime.datetime.strptime(row[2], '%H:%M %d/%m/%Y').strftime('%Y-%m-%d %H:%M')
                    data = ','.join(['"{}"'.format(d) for d in fixed_row])
                    sql = 'INSERT INTO sigcheck VALUES ({});'.format(data)
                    c.execute(sql)

                if len(suspicious) > 0:
                    logger.info('Creating and inserting data for Suspicious...')
                    # Suspicious - table
                    sql = 'CREATE TABLE sus (path TEXT);'
                    c.execute(sql)

                    # Suspicious - data
                    for s in suspicious:
                        sql = 'INSERT INTO sus VALUES("{}");'.format(s)
                        c.execute(sql)

                if got_heuristics:
                    logger.info('Creating and inserting data for Heuristics...')
                    # Heuristics - table
                    sql = 'CREATE TABLE heur (folder TEXT, count INTEGER);'
                    c.execute(sql)
